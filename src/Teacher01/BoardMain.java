...
public class BoardMain {
	...
	private static void updateCategory() {
		...
		if(!categoryList.contains(newCategory)) {
			...
			Board tmp = new Board(category);
			Board tmp1;
			for(int i = 0; i < boardList.size(); i++) {
				tmp1 = boardList.get(i);
				if(tmp1.getCategory().equals(tmp.getCategory())) {
					tmp1.categoryUpdate(newCategory);
				}
			}
			return;
		}
	...
	private static boolean checkLogin(boolean es) {
		if(user != null && es) {
